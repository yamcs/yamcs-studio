...
/**
 * Parse an absolute date/time string. The logic of this date/time parser is not speed optimized. This should not be
 * used for consecutive parses. It should be used in cases of parsing inputs from user.
 *
 * @see #parse(Calendar, String)
 *
 * @author Borut Terpinc Changed the class to use DateTimeFormatter for parsing the date. Formats with zone are
 *         accepted. Added parsers for additional time-stamp formats.
 *
 * @author Sergei Chevtsov developed the original code for the Java Archive Viewer, from which this code heavily
 *         borrows.
 * @author Kay Kasemir
 */
...
public class AbsoluteTimeParser {
    ...
    private static final DateTimeFormatter[] parsers = new DateTimeFormatter[] { // Most complete version first
     ...
     *
     * The milliseconds (sss), seconds(ss), minutes(mm), hours(hh) might be left off, and will then default to zero.
     * <p>
     * When omitting the year, the year from the passed-in calendar is used. When omitting the whole date (YYYY/MM/DD),
     * the values from the passed-in calendar are used. It is not possible to provide <i>only</i> the month
     * <i>without</i> the day or vice versa.
     * <p>
     ...
     *
     * @param cal
     *            Base calendar, defines the time zone as well as the year, in case the text doesn't include a year.
     * @param text
     *            The text to parse.
     ...
     *            used for date
     * @param dateTimeText
     *            date-time string to check and change
    ...
    private static String addDateIfNotProvided(Calendar cal, String dateTimeText) {
        ...
        if (datesep < 0) { // No date at all provided? Use the one from cal.
            dateTimeText = String.format("%04d-%02d-%02d %s",
                    ...
                    dateTimeText);
        } else { // Are there two date separators?
