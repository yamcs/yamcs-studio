...
import org.diirt.datasource.expression.SourceRateReadWriteExpression;
import org.diirt.datasource.expression.WriteExpression;

/**
 * Entry point for the library, manages the defaults and allows to create {@link PVReader}, {@link PVWriter} and
 * {@link PV } from an read or write expression.
 * <p>
 * <b>NotificationExecutor</b> - This is used for all notifications. By default this uses
 * {@link org.diirt.util.concurrent.Executors#localThread()} so that the notification are done on whatever current
 * thread needs to notify. This means that new read notifications are run on threads managed by the
 * ReadScannerExecutorService, write notifications are run on threads managed by the DataSource and exceptions
 * notification are run on the thread where the exception is done. This can be changed to make all notifications routed
 * to single threaded sub-systems, such as UI environments like SWING, SWT or similar. This can be changed on a PV by PV
 * basis.
 * <p>
 * <b>AsynchWriteExecutor</b> - This is used for asynchronous writes, to return right away, and for running timeouts on
 * each write. By default this uses the internal PVManager work pool. The work submitted here is the calculation of the
 * corresponding {@link WriteExpression} and submission to the {@link DataSource}. The DataSource itself typically has
 * asynchronous work, which is executed in the DataSource specific threads. Changing this to
 * {@link org.diirt.util.concurrent.Executors#localThread()} will make that preparation task on the thread that calls
 * {@link PVWriter#write(java.lang.Object) } but it will not transform the call in a synchronous call.
 * <p>
 * <b>ReadScannerExecutorService</b> - This is used to run the periodic scan for new values. By default this uses the
 * internal PVManager work pool. The work submitted here is the calculation of the corresponding
 * {@link DesiredRateExpression} and submission to the NotificationExecutor.
 *
 * @author carcassi
 */
public class PVManager {

    private static volatile Executor defaultNotificationExecutor = org.diirt.util.Executors.localThread();
    ...
    private static final ScheduledExecutorService workerPool = Executors.newScheduledThreadPool(
            ...
            org.diirt.util.Executors.namedPool("PVMgr Worker "));
     ...
     *
     * @param notificationExecutor
     *            the new notification executor
     */
    ...
    /**
     * Reads the given expression, and returns an object to configure the parameters for the read. At each notification
     * it will return the latest value, even if more had been received from the last notification.
     *
     * @param <T>
     *            type of the read payload
     * @param pvExpression
     *            the expression to read
     * @return the read configuration
     */
    public static <T> PVReaderConfiguration<T> read(SourceRateExpression<T> pvExpression) {
        return new PVReaderConfiguration<>(ExpressionLanguage.latestValueOf(pvExpression));
    }
    ...
    public static <T> PVReaderConfiguration<T> read(DesiredRateExpression<T> pvExpression) {
        return new PVReaderConfiguration<>(pvExpression);
    }
    ...
    /**
     * Both reads and writes the given expression, and returns an object to configure the parameters for the both read
     * and write. It's similar to use both {@link #read(org.diirt.datasource.expression.SourceRateExpression) } and
     * {@link #write(org.diirt.datasource.expression.WriteExpression) } at the same time.
     *
     * @param <R>
     *            type of the read payload
     * @param <W>
     *            type of the write payload
     * @param readWriteExpression
    ...
    public static <R, W> PVConfiguration<R, W> readAndWrite(DesiredRateReadWriteExpression<R, W> readWriteExpression) {
        return new PVConfiguration<>(readWriteExpression);
    }
     ...
     *
     * @param readScannerExecutorService
