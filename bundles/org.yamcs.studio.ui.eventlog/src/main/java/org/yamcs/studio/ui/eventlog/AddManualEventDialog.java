...
public class AddManualEventDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		lbl = new Label(container, SWT.NONE);
		lbl.setText("User:");
		if (ConnectionManager.getInstance().isPrivilegesEnabled()) {
			userLbl = new Label(container, SWT.NONE);
			userLbl.setText(ConnectionManager.getInstance().getUsername());
		} else {
	...
	protected void okPressed() {
		...
		EventSeverity severity = EventSeverity.internalGetValueMap().findValueByNumber(severityCombo.getSelectionIndex());
