...
/**
 * Plugin activator, API entry point for obtaining authentication and authorization info
 *
 * @author Kay Kasemir
 */
...
public class SecuritySupport implements BundleActivator {
    ...
    public void start(final BundleContext context) throws Exception {
        ...
        final Collection<ServiceReference<AuthorizationProvider>> authorization_services = context
                .getServiceReferences(AuthorizationProvider.class, filter);
        if (authorization_services.size() == 1) {
            final ServiceReference<AuthorizationProvider> service = authorization_services.iterator().next();
    ...
    /**
     * @return Is it the current, OS-authenticated user? Or is it a different user, authenticated via name/password?
     */
