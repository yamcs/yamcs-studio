...
public class Axis extends LinearScale {
    ...
    final private List<IAxisListener> listeners = new ArrayList<>();
    ...
    protected void layout() {
        ...
        if (isHorizontal() && getTickLablesSide() == LabelSide.Primary) {
            mousePositionLabel.setBounds(new Rectangle(area.x, area.y
                    ...
                    + SPACE_BTW_MARK_LABEL, area.width,
                    area.height
                            - LinearScaleTickMarks.MAJOR_TICK_LENGTH));
    ...
    /**
     * @return Range that reflects the minimum and maximum value of all traces on this axis. Returns <code>null</code>
    ...
    /**
     * Perform an auto-scale: Axis limits are set to the value range of the traces on this axis. Includes some
     * optimization: Axis range is set a little wider than exact trace data range. When auto-scale would only perform a
     * minor axis adjustment, axis is left unchanged.
     *
     * @param force
     *            If true, the axis will be auto-scaled by force regardless the autoScale field. Otherwise, it will use
     *            the autoScale field to judge whether an auto-scale will be performed.
    ...
    /**
     * The autoScaleThreshold must be a value in range [0,1], which represents a percentage of the plot area for the
     * threshold when autoScale is performed.The autoScale will performed only if the spare space exceeds this
     * threshold. So it can reduce the CPU usage by increasing the threshold.
     * 
     ...
     * 
     * @param onPrimarySide
     *            set true if the axis on primary side(Bottom/Left); false if it is not on the primary side of xy
     *            graph(Top/Right).
     */
    ...
    /**
     * @return true if the axis is on the primary side of xy graph(Bottom/Left); false if it is on the secondary
     *         side(Top/Right).
     */
    ...
    /**
     * Listener to mouse events, performs panning and some zooms Is very similar to the PlotMouseListener, but unclear
