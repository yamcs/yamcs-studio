...
public class TMScale extends JSlider {
    ...
    void setToZoom(ZoomSpec zoom) {
        ...
        if (measure > 1 * 365 * 86400) {
            ...
            for (; (instant = cal.getTimeInMillis()) > zoom.startInstant; cal.add(Calendar.MONTH, -1)) {
            ...
            for (; (instant = cal.getTimeInMillis()) > zoom.startInstant;) {
                if (d == 1) {
                    f = f_yyyy_MM_dd;
                } else {
                    f = f_dd;
            ...
            cal.set(Calendar.HOUR_OF_DAY, 0);
            cal.set(Calendar.MINUTE, 0);
            for (; (instant = cal.getTimeInMillis()) > zoom.startInstant; cal.add(Calendar.DAY_OF_YEAR, -1)) {
            ...
            cal.set(Calendar.MINUTE, 0);
            for (; (instant = cal.getTimeInMillis()) > zoom.startInstant; cal.add(Calendar.HOUR_OF_DAY, -1)) {
            ...
            for (; (instant = cal.getTimeInMillis()) > zoom.startInstant; cal.add(Calendar.MINUTE, -15)) {
            ...
            for (; (instant = cal.getTimeInMillis()) > zoom.startInstant; cal.add(Calendar.MINUTE, -1)) {
    ...
    /**
     * Tricky to change JSlider font size See
     * http://nadeausoftware.com/articles/2009/04/mac_java_tip_how_customize_aqua_sliders
     */
