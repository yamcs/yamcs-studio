...
/**
 * Forked from org.csstudio.utility.product.StartupParameters to remove dependency on utility.product
 */
public class StartupParameters implements StartupParametersExtPoint {
    ...
    /**
     * Parameter tag which defines if login dialog should be displayed The value is stored in the returned map.
     */
    public static final String LOGIN_PROMPT_PARAM = "css.showLogin";

    /**
     * Parameter tag which defines if prompt for workspace is forced. The value is stored in the returned map.
     */
        ...
        for (int i = 0; i < args.length; ++i) {
            ...
            if (arg.equalsIgnoreCase(HELP) || arg.equalsIgnoreCase("-?")) {
                ...
                if ((i + 1) < args.length) {
                    ...
                    if (!next.startsWith("-")) {
                        default_workspace = new URL("file:" + next);
                        ++i;
                    }
                }
    ...
    private void showHelp() {
        ...
        System.out.format(
                "  %-40s : set a password preferences (will prompt for password or use previous -p {password} option)\n",
