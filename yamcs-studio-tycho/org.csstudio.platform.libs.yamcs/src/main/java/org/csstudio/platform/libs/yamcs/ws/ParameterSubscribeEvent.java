...
public class ParameterSubscribeEvent extends OutgoingEvent {
    
    private Set<NamedObjectId> ids = new HashSet<>();

    public ParameterSubscribeEvent(NamedObjectList idList) {
    ...
    public boolean canMergeWith(OutgoingEvent otherEvent) {
        return otherEvent instanceof ParameterSubscribeEvent;
    }
    
    @Override
    public OutgoingEvent mergeWith(OutgoingEvent otherEvent) {
        Set<NamedObjectId> otherIds = ((ParameterSubscribeEvent) otherEvent).ids;
        ids.addAll(otherIds);
        return this;
    }
}
