...
/**
 * Controls yamcs command queues via Hornet Allows to register one CommandQueueListener per
 * (instance, yprocessor)
 *
 * @author nm
 *
...
public class CommandQueueControlClient implements ConnectionListener {
    ...
    private static final Logger log = Logger.getLogger(CommandQueueControlClient.class.getName());
    ...
    public void receiveInitialConfig() {
        try {
            ...
            YamcsClient browser = yconnector.getSession().newClientBuilder().setDataConsumer(CMDQUEUE_INFO_ADDRESS, CMDQUEUE_INFO_ADDRESS)
    ...
    /**
     * Send a message to the server to change the queue state. If newState=ENABLED, then changing
     * the queue state may result in some commands being sent. The rebuild flag indicates that the
     * command shall be rebuild (new timestamp, new pvt checks, etc)
     */
