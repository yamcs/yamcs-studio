...
public class XYGraphModel extends AbstractPVWidgetModel {
    ...
    public final static String[] TIME_FORMAT_ARRAY = new String[] { "None", "yyyy-MM-dd\nHH:mm:ss",
            "yyyy-MM-dd\nHH:mm:ss.SSS", "HH:mm:ss", "HH:mm:ss.SSS", "HH:mm", "yyyy-MM-dd", "MMMMM d", "Auto" };
    ...
    public final static String[] AXES_ARRAY = new String[MAX_AXES_AMOUNT];
    {
        ...
        for (var i = 2; i < MAX_AXES_AMOUNT; i++) {
    ...
    protected void configureProperties() {
        addPVProperty(new PVNameProperty(PROP_TRIGGER_PV, "Trigger PV", WidgetPropertyCategory.Behavior, ""),
        ...
        addProperty(new FontProperty(PROP_TITLE_FONT, "Title Font", WidgetPropertyCategory.Display,
        ...
        addProperty(new IntegerProperty(PROP_AXIS_COUNT, "Axis Count", WidgetPropertyCategory.Behavior, 2, 2,
                MAX_AXES_AMOUNT));
        addProperty(new IntegerProperty(PROP_TRACE_COUNT, "Trace Count", WidgetPropertyCategory.Behavior, 1, 0,
                MAX_TRACES_AMOUNT));
        addAxisProperties();
        addTraceProperties();
    ...
    public void processVersionDifference(Version boyVersionOnFile) {
        ...
        if (UpgradeUtil.VERSION_WITH_PVMANAGER.compareTo(boyVersionOnFile) > 0) {
            setPropertyValue(PROP_TRIGGER_PV,
                    UpgradeUtil.convertUtilityPVNameToPM((String) getPropertyValue(PROP_TRIGGER_PV)));

            for (var i = 0; i < MAX_TRACES_AMOUNT; i++) {
                var traceXPVPropId = makeTracePropID(TraceProperty.XPV.propIDPre, i);
                setPropertyValue(traceXPVPropId,
                        UpgradeUtil.convertUtilityPVNameToPM((String) getPropertyValue(traceXPVPropId)));

                var traceYPVPropId = makeTracePropID(TraceProperty.YPV.propIDPre, i);
                setPropertyValue(traceYPVPropId,
                        UpgradeUtil.convertUtilityPVNameToPM((String) getPropertyValue(traceYPVPropId)));
            }
        }
    }

    private void addAxisProperties() {
        for (var i = 0; i < MAX_AXES_AMOUNT; i++) {
    ...
    private void addAxisProperty(AxisProperty axisProperty, int axisIndex, WidgetPropertyCategory category) {
        var propID = makeAxisPropID(axisProperty.propIDPre, axisIndex);
        ...
        case MIN:
            ...
            break;
        case TIME_FORMAT:
            addProperty(new ComboProperty(propID, axisProperty.toString(), category, TIME_FORMAT_ARRAY, 0));
            break;
        case GRID_COLOR:
    ...
    private void addTraceProperty(TraceProperty traceProperty, int traceIndex) {
        ...
        case ANTI_ALIAS:
            ...
            break;
        case BUFFER_SIZE:
            addProperty(new IntegerProperty(propID, traceProperty.toString(), category, DEFAULT_BUFFER_SIZE, 1,
                    MAX_BUFFER_SIZE));
            break;
        ...
        case YPV:
            addPVProperty(
                    new PVNameProperty(propID, traceProperty.toString(), category,
