...
public class BoolSwitchFigure extends AbstractBoolControlFigure {
    ...
    class Bar extends Figure {
        ...
        public boolean containsPoint(int x, int y) {
            ...
            var smallEndEllipse = new Ellipse();
            smallEndEllipse.setBounds(smallEndBounds);
            var bigEndEllipse = new Ellipse();
        ...
        protected void paintFigure(Graphics graphics) {
            ...
            if (!horizontal) { // if vertical
                echelon = new PointList(new int[] { bigEndBounds.x, bigEndBounds.y + bigEndBounds.height / 2,
                ...
                if (effect3D && support3D) {
                    pattern = GraphicsUtil.createScaledPattern(graphics, Display.getCurrent(), bigEndBounds.x,
                            bigEndBounds.y, bigEndBounds.x + bigEndBounds.width, bigEndBounds.y, BLACK_COLOR, 10,
                ...
                echelon = new PointList(new int[] { bigEndBounds.x + bigEndBounds.width / 2, bigEndBounds.y,
                ...
                if (effect3D && support3D) {
                    pattern = GraphicsUtil.createScaledPattern(graphics, Display.getCurrent(), bigEndBounds.x,
                            bigEndBounds.y, bigEndBounds.x, bigEndBounds.y + bigEndBounds.height, BLACK_COLOR,
                            booleanValue ? 0 : 10, BLACK_COLOR, booleanValue ? 150 : 220);
                    graphics.setBackgroundPattern(pattern);
                    graphics.fillOval(smallEndBounds);
                }
    ...
    class BoolSwitchLayout extends AbstractLayout {
        ...
        private void horizontalLayout(IFigure container) {
            ...
            if (bar != null && bar.isVisible()) {
                var bigEndD = new Dimension((int) ((35.0 / 218.0) * W), (int) ((45.0 / 105.0) * H));
                var smallEndD = new Dimension((int) ((43.0 / 218.0) * W), (int) ((35.0 / 105.0) * H));

                var smallMove = (int) ((1.0 / 7.0) * pedBounds.width);
                smallMove = booleanValue ? -smallMove : smallMove;
                if (!booleanValue) {
                    bigBounds = new Rectangle(0, pedBounds.height / 2 - bigEndD.height / 2, bigEndD.width,
                            bigEndD.height);
                } else {
                    bigBounds = new Rectangle(2 * pedBounds.x + pedBounds.width - bigEndD.width,
                            pedBounds.height / 2 - bigEndD.height / 2, bigEndD.width, bigEndD.height);
                }
                smallBounds = new Rectangle(pedBounds.x + pedBounds.width / 2 - smallEndD.width / 2,
                        pedBounds.y + pedBounds.height / 2 - smallEndD.height / 2, smallEndD.width, smallEndD.height);
            ...
            if (shadow != null && shadow.isVisible()) {
                if (booleanValue) {
                    shadow.setBounds(new Rectangle(smallBounds.x + smallBounds.width / 2, smallBounds.y,
                            W - smallBounds.x - smallBounds.width / 2, H - smallBounds.y));
                } else {
                    shadow.setBounds(new Rectangle((int) ((34.0 / 218.0) * W), smallBounds.y,
                            smallBounds.x + smallBounds.width - (int) ((34.0 / 218.0) * W), H - smallBounds.y));
                }
            }
            if (boolLabel != null && boolLabel.isVisible()) {
                var labelSize = boolLabel.getPreferredSize();
                boolLabel.setBounds(new Rectangle(
                        getLabelLocation(pedBounds.x + pedBounds.width / 2 - labelSize.width / 2,
        ...
        private void verticalLayout(IFigure container) {
            ...
            if (bar != null && bar.isVisible()) {
                var bigEndD = new Dimension((int) ((45.0 / 105.0) * W), (int) ((35.0 / 218.0) * H));
                var smallEndD = new Dimension((int) ((35.0 / 105.0) * W), (int) ((43.0 / 218.0) * H));

                var smallMove = (int) ((1.0 / 7.0) * pedBounds.height);
                smallMove = booleanValue ? -smallMove : smallMove;
                if (booleanValue) {
                    barBounds = new Rectangle(pedBounds.width / 2 - bigEndD.width / 2, 0, bigEndD.width,
                            pedBounds.y + pedBounds.height / 2 + smallEndD.height / 2 + 2);
                    bigBounds = new Rectangle(barBounds.x, 0, bigEndD.width, bigEndD.height);
                } else {
                    barBounds = new Rectangle(pedBounds.width / 2 - bigEndD.width / 2,
                            pedBounds.y + pedBounds.height / 2 - smallEndD.height / 2, bigEndD.width,
                            pedBounds.y + pedBounds.height / 2 + smallEndD.height / 2 + 2);
                    bigBounds = new Rectangle(barBounds.x, barBounds.y + barBounds.height - bigEndD.height,
            ...
            if (shadow != null && shadow.isVisible()) {
                if (!booleanValue) {
                    shadow.setBounds(new Rectangle(smallBounds.x, smallBounds.y + smallBounds.height / 2,
                            W - smallBounds.x, H - smallBounds.y - smallBounds.height / 2));
                } else {
                    shadow.setBounds(new Rectangle(smallBounds.x, (int) ((34.0 / 218.0) * H), W - smallBounds.x,
            ...
            if (effect3D && support3D) {
                ...
                if (booleanValue) {
                    pattern = GraphicsUtil.createScaledPattern(graphics, Display.getCurrent(), bounds.x, bounds.y,
                            bounds.x + bounds.width, bounds.y + bounds.height, WHITE_COLOR, 10, BLACK_COLOR, 100);
                } else {
                    pattern = GraphicsUtil.createScaledPattern(graphics, Display.getCurrent(), bounds.x, bounds.y,
                            bounds.x + bounds.width, bounds.y + bounds.height, BLACK_COLOR, 0, BLACK_COLOR, 150);
                }
                graphics.setBackgroundPattern(pattern);
                graphics.fillOval(bounds);
                pattern.dispose();
                ...
                if (booleanValue) {
                    if (horizontal) {
                        pattern = GraphicsUtil.createScaledPattern(graphics, Display.getCurrent(), bounds.x, bounds.y,
                                bounds.x, bounds.y + bounds.height, BLACK_COLOR, 5, BLACK_COLOR, 50);
                    } else {
                        pattern = GraphicsUtil.createScaledPattern(graphics, Display.getCurrent(), bounds.x, bounds.y,
                                bounds.x + bounds.width, bounds.y, BLACK_COLOR, 5, BLACK_COLOR, 100);
                    }
                    graphics.setBackgroundPattern(pattern);
                    graphics.fillOval(bounds);
                    pattern.dispose();
        ...
        protected void paintClientArea(Graphics graphics) {
            ...
            if (horizontal) {
                if (!booleanValue) {
                    ...
                    var smallEndBounds = bar.getSmallEndBounds();
                    var echelonShadow = new int[] { p1.x, p1.y, p2.x, p2.y, smallEndBounds.x + smallEndBounds.width,
                            smallEndBounds.y + smallEndBounds.height / 2, smallEndBounds.x + smallEndBounds.width / 2,
                            smallEndBounds.y };
                    gc.setBackground(GRAY_COLOR);

                    gc.fillRectangle(image.getBounds());
                    var tr = new Transform(Display.getCurrent());
                    ...
                    var smallEndBounds = bar.getSmallEndBounds();
                    var echelonShadow = new int[] { p1.x, p1.y, p2.x, p2.y, smallEndBounds.x + smallEndBounds.width / 2,
                            smallEndBounds.y + smallEndBounds.height, smallEndBounds.x + smallEndBounds.width / 2,
                            smallEndBounds.y };
                    gc.setBackground(GRAY_COLOR);
                    gc.fillRectangle(image.getBounds());
                ...
                if (booleanValue) {
                    ...
                    var p1 = new Point((int) ((60.0 / 95.0) * bounds.width), bounds.y + 3);

                    var p2 = new Point(p1.x + (int) ((20.0 / 95.0) * bounds.width),
                            p1.y + (int) ((20.0 * Math.sqrt(3.0) / 95.0) * bounds.width));
                    var smallEndBounds = bar.getSmallEndBounds();
                    var echelonShadow = new int[] { p1.x, p1.y, p2.x, p2.y, smallEndBounds.x + smallEndBounds.width / 2,
                            smallEndBounds.y + smallEndBounds.height, smallEndBounds.x,
                            smallEndBounds.y + smallEndBounds.height / 2 };
                    gc.setBackground(GRAY_COLOR);
    ...
    private final Color GRAY_COLOR = CustomMediaFactory.getInstance().getColor(new RGB(200, 200, 200));

    private final Color WHITE_COLOR = CustomMediaFactory.getInstance().getColor(CustomMediaFactory.COLOR_WHITE);
    private final Color BLACK_COLOR = CustomMediaFactory.getInstance().getColor(CustomMediaFactory.COLOR_BLACK);
